"""
ACL 2023-2026 专用抓取脚本。
1. 从 ACL Anthology GitHub XML 获取所有论文标题、作者和摘要
2. 按关键词筛选 AI 安全相关论文
...
logger = logging.getLogger("ACL_Scrape")


def main():
    acl_config = CONFERENCES["ACL"]
    scraper = get_scraper(acl_config)
    ...
    for year in [2023, 2024, 2025, 2026]:
        logger.info(f"=== ACL {year} ===")

        # Step 1: 从 XML 获取所有论文（含标题、作者、摘要）
        all_papers = scraper.scrape(year)
        logger.info(f"  Total papers from ACL Anthology XML: {len(all_papers)}")
    ...
    # Step 3: 合并到现有数据
    logger.info(f"=== Merging {len(all_new_security_papers)} new ACL papers into existing {len(existing_papers)} ===")
